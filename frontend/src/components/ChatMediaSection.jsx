...
const ChatMediaSection = ({ userToChatId, onClose }) => {
  ...
  useEffect(() => {
    fetchGroupMedia();
  }, [page, activeFilter]); // Fetch media when filter changes

  const fetchGroupMedia = async () => {
    setLoading(true);
    try {
      const res = await axiosInstance.get(`/messages/${selectedUser._id}/media?page=${page}&limit=9&filter=${activeFilter}`);
            ...
            {Object.keys(groupedMedia).length > 0 ? (
              Object.keys(groupedMedia).map((dateString) => (
                <div key={dateString} className="pb-2">
                  <h4 className="font-semibold pb-2">{dateString}</h4>
