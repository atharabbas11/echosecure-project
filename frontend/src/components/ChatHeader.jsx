...
const ChatHeader = ({ onHeaderClick, onChatClose }) => {
  const { selectedUser, setSelectedUser, setExpiryTime, expiryTime } = useChatStore();
  ...
  useEffect(() => {
    const fetchExpirySetting = async () => {
      ...
      try {
        ...
        const selectedUserSetting = disappearSettings[selectedUser._id] || "off"; // Default to 'off' if no setting exists
