...
const MessageInput = () => {
  ...
  const { sendMessage, repliedTo, setRepliedTo, messages, selectedUser, getUsersForContact, users } = useChatStore();
  ...
  // Reset all state when selectedUser changes
  useEffect(() => {
    setText("");
    ...
    <div className="p-4 w-full bg-transparent">
            ...
            <div className="max-h-60 overflow-y-auto">
              {users.map((user) => (
                <div key={user._id} className="flex gap-2 p-2 hover:bg-gray-100 cursor-pointer" onClick={() => handleSelectContact(user)}>
                  ...
                  <p className="text-center mt-2 mb-2">{user.fullName}</p>
                </div>
              ))}
            </div>
      ...
      {videoPreview && (
        <div className="mb-3 relative">
          ...
          <button onClick={handleVideoRemove} className="absolute top-0 right-0 -mt-2 -mr-2 w-6 h-6 rounded-full bg-base-300 flex items-center justify-center border border-gray-300" type="button">
            <X className="text-gray-700" size={16} />
          </button>
        </div>
      )}
      ...
      {audioBlob && (
        <div className="mb-3">
          ...
          <button onClick={() => setAudioBlob(null)} className="mt-2 text-red-500">
            Remove Document
          </button>
        </div>
      )}
      ...
      <form onSubmit={handleSendMessage} className="flex items-center gap-2">
        <div className="flex-1 flex gap-2">
          <textarea className="w-full p-3 text-sm placeholder:text-gray-400 input input-bordered rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Type a message..." value={text} onChange={(e) => setText(e.target.value)} style={{ backgroundColor: "transparent", scrollbarWidth: "thin", scrollbarColor: "#5A67D8 transparent" }}/>
