...
const GroupMedia = ({ groupId, onSeeMoreClick }) => {
  ...
  const fetchGroupMedia = async () => {
    ...
    setLoading(true);
    try {
      const res = await axiosInstance.get(`/groups/${groupId}/media?page=${page}&limit=3&filter=${activeFilter}`);
      // console.log("Group Media: ",res);
